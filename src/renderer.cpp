...
void Renderer::init_effects_FBO() {
    ...
    glGenFramebuffers(1, &effects_FBO);TEST_OPENGL_ERROR();
    glBindFramebuffer(GL_FRAMEBUFFER, effects_FBO);TEST_OPENGL_ERROR();
    // on genere/regle les textures
    glGenTextures(2, effects_buffers);TEST_OPENGL_ERROR();
    for (int i = 0; i < 2; i++) {
        glBindTexture(GL_TEXTURE_2D, effects_buffers[i]);TEST_OPENGL_ERROR();
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0 + i, GL_TEXTURE_2D, effects_buffers[i], 0);TEST_OPENGL_ERROR();
        glBindImageTexture(1 + i, effects_buffers[i], 0, GL_FALSE, 0, GL_READ_WRITE, GL_RGBA32F);
    }
...
void Renderer::init_color_FBO() {
    ...
    */
    ...
    // on genere/regle les textures
    glGenTextures(1, &color_buffer);TEST_OPENGL_ERROR();
    glBindTexture(GL_TEXTURE_2D, color_buffer);TEST_OPENGL_ERROR();
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);TEST_OPENGL_ERROR();
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);TEST_OPENGL_ERROR();
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);TEST_OPENGL_ERROR();
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);TEST_OPENGL_ERROR();
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, width, height,
                 0, GL_RGBA, GL_FLOAT, nullptr);TEST_OPENGL_ERROR();
    // on lie les deux textures aux color attachments
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, color_buffer, 0);TEST_OPENGL_ERROR();
...
bool Renderer::init_shader_threshold() {
    if (!threshold_prog.add_shader("vertex.shd", GL_VERTEX_SHADER) ||
        !threshold_prog.add_shader("fragment_threshold.shd", GL_FRAGMENT_SHADER))
        return false;

    if (!threshold_prog.link())
        return false;
